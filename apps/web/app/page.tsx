...
export default function HomePage() {
  ...
  const [lesson, setLesson] = useState<LessonRequest>({
    grade: 8,
  ...
  // Load the catalogue and align the opening selection with what it actually contains.
  useEffect(() => {
    api
      .catalog()
      .then((loaded) => {
        setCatalog(loaded);
        setLesson((current) => {
          const grade = loaded.grades.find((entry) => entry.grade === current.grade) ?? loaded.grades[0];
          const topic = grade?.topics.find((entry) => entry.id === current.topic_id) ?? grade?.topics[0];
          const subtopic =
            topic?.subtopics.find((entry) => entry.id === current.subtopic_id) ?? topic?.subtopics[0];
          if (!grade || !topic || !subtopic) return current;
          return { ...current, grade: grade.grade, topic_id: topic.id, ...fromSubtopic(subtopic) };
        });
      })
  ...
  const selectGrade = useCallback(
    (grade: number) => {
      ...
      setLesson((current) => ({ ...current, grade, topic_id: topic.id, ...fromSubtopic(subtopic) }));
    },
    [catalog],
  );
  ...
  const selectTopic = useCallback(
    (topicId: string) => {
      ...
      const subtopic = topic?.subtopics[0];
      if (!subtopic) return;
      setLesson((current) => ({ ...current, topic_id: topicId, ...fromSubtopic(subtopic) }));
    },
  ...
  const selectSubtopic = useCallback(
    (subtopicId: string) => {
      ...
      if (!subtopic) return;
      setLesson((current) => ({ ...current, ...fromSubtopic(subtopic) }));
    },
        ...
        <div className="header-inner">
          <a className="brand" href="#top">
            ...
            <span>
              ...
              <span className="brand-sub">Maths micro-lessons</span>
            </span>
          </a>
      ...
      <main className="main" id="top">
        ...
        {step === "setup" && (
          <SetupStep
            ...
            lesson={lesson}
            onGrade={selectGrade}
            onLevel={(level: Level) => setLesson((current) => ({ ...current, level }))}
            onMethod={(method: Method) => setLesson((current) => ({ ...current, method }))}
            onObjective={(objective) => setLesson((current) => ({ ...current, objective }))}
