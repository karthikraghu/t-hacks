...
A section that needs a full explanatory sentence on screen is a statement section, because a sentence
does not fit alongside a diagram.
...
def section_3(self, duration):
    ...
    self.play(GrowArrow(weight), GrowArrow(normal), FadeIn(legend), run_time=1.4)
